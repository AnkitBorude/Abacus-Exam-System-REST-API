...
const examValidationschema = Joi.object({
    title: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'title can only contain alphanumeric characters, spaces, #, and :.',
            'string.min': 'title must be at least 3 characters long.',
            'string.max': 'title cannot exceed 100 characters.',
        }),

    duration: Joi.number().integer().min(1).max(360).messages({
    ...
    isSingleAttempt: Joi.boolean().messages({
        'boolean.base':
            'isSingleAttempt must be a boolean value (true or false).',
    }),
...
const examValidation = (req, res, next) => {
    ...
    if (req.method === 'POST') {
        ...
        let finalValidationSchema = examPostvalidationSchema.concat(
