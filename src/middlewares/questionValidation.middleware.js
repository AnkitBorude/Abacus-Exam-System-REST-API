...
let questionValidationschema = Joi.object({
    minNumber: Joi.number().integer().min(1).max(999).required().messages({
    ...
    maxTerms: Joi.number().integer().min(2).max(8).required().messages({
    ...
    operators: Joi.array()
        .items(
            Joi.string().valid('+', '-', '/', '*').required().messages({
                'any.only':
                    'operators can only include + (plus sign) , - (minus sign/ hyphen), / (forward slash), or * (star/ asterisk).',
            })
        )
        .min(1)
        .max(4)
