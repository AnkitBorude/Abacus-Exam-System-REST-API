...
const mcqGenerator = (config, totalQuestions = 1, marks = 1) => {
    ...
    while (i < totalQuestions) {
        ...
        if (expressionAnswer > 5) {
            //if the answer is greter than 5 then add to questions list only
...
const validateConfig = (config) => {
    ...
    if (config.minNumber <= 1) {
        throw new Error(
            'Invalid minNumber: must be a number greater than 1 and non-negative.'
        );
    }
    ...
    for (const operator of config.operators) {
        if (
            ...
            !allowableOperators.includes(operator)
        ) {
            throw new Error(
                `Invalid operator: "${operator}" is not one of the allowed operators: ${allowableOperators.join(', ')}.`
            );
        }
    }
    ...
    if (config.totalQuestions < 1) {
        throw new Error(
            'Invalid totalQuestions: must be a number greater than or equal to 1.'
        );
    }
};
